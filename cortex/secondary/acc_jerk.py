...
def acc_jerk(threshold=500, **kwargs):
    ...
    if _acc:
        ...
        acc_df['dt'] = acc_df['timestamp'] - acc_df['timestamp'].shift()
        acc_df['x_shift'] = acc_df['x'].shift()
        acc_df['y_shift'] = acc_df['y'].shift()
        acc_df['z_shift'] = acc_df['z'].shift()
        acc_df = acc_df[acc_df['dt'] < threshold]
        x_sum = np.square((acc_df['x'] - acc_df['x_shift']) / acc_df['dt']).sum(axis=1)
        y_sum = np.square((acc_df['y'] - acc_df['y_shift']) / acc_df['dt']).sum(axis=1)
        z_sum = np.square((acc_df['z'] - acc_df['z_shift']) / acc_df['dt']).sum(axis=1)
