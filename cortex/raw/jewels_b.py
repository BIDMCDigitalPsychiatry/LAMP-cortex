...
def jewels_b(resolution=None, limit=20000, **kwargs):
    jewels_b_ids = [activity['id'] for activity in LAMP.Activity.all_by_participant(kwargs['id'])['data'] if activity['spec'] == 'lamp.jewels_b']
    _jewels_b = [{'timestamp':res['timestamp'],
                                ...
                                'temporal_slices':res['temporal_slices']} for res in LAMP.ActivityEvent.all_by_participant(kwargs['id'], 
                                                                                                                           _from=kwargs['start'],
                                                                                                                           to=kwargs['end'],
                                                                                                                           _limit=limit)['data'] if res['activity'] in jewels_b_ids]
